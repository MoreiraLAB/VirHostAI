...
def write_sets_to_files(interactions, outdir, seq_mapping, tag=""):
    ...
    with open(os.path.join(outdir, f'unique_prot_c{tag}_40%.txt'), 'w') as f:
        f.write('\n'.join(sorted(prot_c)))
    with open(os.path.join(outdir, f'unique_prot{tag}_40%.txt'), 'w') as f:
        f.write('\n'.join(sorted(prot)))
    with open(os.path.join(outdir, f'unique_ligands{tag}_40%.txt'), 'w') as f:
        f.write('\n'.join(sorted(ligs)))
    with open(os.path.join(outdir, f'unique_ligand_ids{tag}_40%.txt'), 'w') as f:
        f.write('\n'.join(sorted(lig_no_chain_set)))
    with open(os.path.join(outdir, f'unique_sequences{tag}_40%.txt'), 'w') as f:
        f.write('\n'.join(sorted(seqs)))
    with open(os.path.join(outdir, f'unique_prot_c_lig{tag}_40%.txt'), 'w') as f:
...
def split_all(
    ...
    val_size=0.1,
    seed=44,
    mmseqs_clusters_path="mmseqs2_40clusters.txt"
):
    ...
    # Save splits
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_train_40%.hdf5"), train_ids, "train")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_val_40%.hdf5"),   val_ids,   "val")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_test_40%.hdf5"),  test_ids, "test")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_blind_40%.hdf5"), blind_ids, "blind")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_blind_protein_40%.hdf5"), blind_protein_ids, "blind_protein")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_blind_ligand_40%.hdf5"),  blind_ligand_ids,  "blind_ligand")
...
split_all("viralbindpredictDB-prottrans-cleaned.hdf5", "deduplicated_filtered_prottrans_df.xlsx", output_dir="splits_prottrans_output_40%", test_size=0.1, blind_frac=0.1, val_size=0.1, seed=52, mmseqs_clusters_path="mmseqs2_40clusters.txt")
...
# Validate splits
for split_file in [
    'prottrans_train_40%.hdf5',
    'prottrans_test_40%.hdf5',
    'prottrans_val_40%.hdf5',
    'prottrans_blind_40%.hdf5',
    'prottrans_blind_ligand_40%.hdf5',
    'prottrans_blind_protein_40%.hdf5'
]:
    validate_first3_shapes(os.path.join("splits_prottrans_output_40%", split_file))