...
def clusters_of_interactions(interaction_ids, lig_to_cluster):
    """Return set of scaffold-cluster IDs used by a list of interaction IDs."""
    cids = set()
    for i in interaction_ids:
        lig_base = lig_no_chain(i)
        cid = lig_to_cluster.get(lig_base)
        if cid is not None:
            cids.add(cid)
    return cids
...
def write_sets_to_files(interactions, outdir, seq_mapping, tag=""):
    ...
    with open(os.path.join(outdir, f'unique_prot_c{tag}_90%.txt'), 'w') as f:
        f.write('\n'.join(sorted(prot_c)))
    with open(os.path.join(outdir, f'unique_prot{tag}_90%.txt'), 'w') as f:
        f.write('\n'.join(sorted(prot)))
    with open(os.path.join(outdir, f'unique_ligands{tag}_90%.txt'), 'w') as f:
        f.write('\n'.join(sorted(ligs)))
    with open(os.path.join(outdir, f'unique_ligand_ids{tag}_90%.txt'), 'w') as f:
        f.write('\n'.join(sorted(lig_no_chain_set)))
    with open(os.path.join(outdir, f'unique_sequences{tag}_90%.txt'), 'w') as f:
        f.write('\n'.join(sorted(seqs)))
    with open(os.path.join(outdir, f'unique_prot_c_lig{tag}_90%.txt'), 'w') as f:
    ...
    with open(mmseqs_file_path, 'r') as file:
        for line in file:
            ...
            if line.startswith(">"):
                ...
                # Convert to PDB:Chain (expected downstream)
                if "_" in entry:
...
def split_all(
    ...
    pdb_chain_to_cluster = load_mmseqs_clusters("mmseqs2_90clusters.txt")
    ...
    # Ligand → SMILES → EXACT BM scaffold; acyclics produce None and are not used for blind
    smiles_df = pd.read_excel(seq_mapping_excel)
    ligand_to_smiles = dict(zip(smiles_df['LIGAND_ID'], smiles_df['SMILES']))
    ...
    def bm_scaffold_exact(smi):
        ...
        try:
            ...
            return None  # acyclic or empty scaffold
        except Exception:
            return None
    ...
    print(f"[Scaffolds] Ligands present in interactions (with scaffold): {len(lig_to_scaffold)}")
    print(f"[Scaffolds] Unique exact BM scaffolds: {len(unique_scaffolds)}")

    # Sample blind protein clusters (MMseqs)
    unique_clusters = df['ClusterID'].dropna().unique()
    if len(unique_clusters) == 0:
        print("[WARNING] No MMseqs clusters found; blind_protein will be empty.")
    ...
    # Sample blind scaffolds and derive blind ligands
    if len(unique_scaffolds) == 0:
        print("[WARNING] No ligands with BM scaffolds; blind_ligand will be empty.")
    ...
    print(f"[Scaffolds] Sampled {len(sampled_scaffolds)} scaffolds → "
    ...
    # Group by (PDB:sequence:ligand) so related interactions move together
    ...
    # TRAIN/VAL/TEST split on clean candidates, grouped by (PDB:seq:lig)
    ...
    # Assert: no scaffold leakage into train/val
    ...
    # Save splits
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_train_90%.hdf5"), train_ids, "train")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_val_90%.hdf5"),   val_ids,   "val")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_test_90%.hdf5"),  test_ids, "test")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_blind_90%.hdf5"), blind_ids, "blind")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_blind_protein_90%.hdf5"), blind_protein_ids, "blind_protein")
    save_split_hdf5(original_hdf5, os.path.join(output_dir, "prottrans_blind_ligand_90%.hdf5"),  blind_ligand_ids,  "blind_ligand")
...
split_all("viralbindpredictDB-prottrans-cleaned.hdf5", "deduplicated_filtered_prottrans_df.xlsx", output_dir="splits_prottrans_output_90%", test_size=0.1, blind_frac=0.1, val_size=0.1, seed=27)
...
# Validate splits
for split_file in [
    'prottrans_train_90%.hdf5',
    'prottrans_test_90%.hdf5',
    'prottrans_blind_90%.hdf5',
    'prottrans_blind_ligand_90%.hdf5',
    'prottrans_blind_protein_90%.hdf5'
]:
    validate_first3_shapes(os.path.join("splits_prottrans_output_90%", split_file))