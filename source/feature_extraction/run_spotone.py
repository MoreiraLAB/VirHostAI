...
"""
from source.feature_extraction.ViralBindPredict_variables import HEADER_COLUMNS, ONE_TO_THRE_CODE_CONVERTER, \
...
class SpotOne():
    ...
    def generate_encoded(self, input_sequence):
    ...
    def transform_file(self, input_file):
    ...
    def generate_table_file(self, input_dict, output_file):
    ...
    def location_features(self, input_sequence, target_residue):
    ...
    def extract_amino_properties(self, input_res, retrieved_header = False):
    ...
    def extract_subsections(self, res_number, input_dict):
    ...
    def extract_centred(self, input_dict, res_number, features_table):

        output_features = []
        calculated_sections = self.extract_subsections(res_number, input_dict)
    ...
    def extract_sections(self, input_dict, features_table):
    ...
    def render_discriminated_sequence_dict(self, input_sequence, chain, first_input = True):
    ...
    def generate_simple_features(self, input_dict, input_file, chains_original, input_raw_file):
        ...
        for index, row in opened_file.iterrows():
            ...
            relative_position = self.location_features(input_dict[current_chain], row[RES_NUMBER_COL])
            if (row[COMPLEX_NAME_COL] in list(discriminated_dictionary.keys())) and (row[CHAIN_NAME_COL] not in list(discriminated_dictionary[row[COMPLEX_NAME_COL]].keys())):
                discriminated_dictionary[row[COMPLEX_NAME_COL]][row[CHAIN_NAME_COL]] = self.render_discriminated_sequence_dict(input_dict[current_chain], row[CHAIN_NAME_COL], first_input = False)
            elif row[COMPLEX_NAME_COL] not in list(discriminated_dictionary.keys()):
                discriminated_dictionary[row[COMPLEX_NAME_COL]] = self.render_discriminated_sequence_dict(input_dict[current_chain], row[CHAIN_NAME_COL], first_input = True)
            window_centred = self.extract_centred(discriminated_dictionary[row[COMPLEX_NAME_COL]][row[CHAIN_NAME_COL]], str(row[RES_NUMBER_COL]), template_features_table)
            if written_header == True:
                table_properties = self.extract_amino_properties(row[RES_NAME_COL], retrieved_header = True)
            if written_header == False:
                header_properties, table_properties = self.extract_amino_properties(row[RES_NAME_COL], retrieved_header = False)
    ...
    def execute(self, protein_file):

        path = SPOTONE_FOLDER

        test_file = path+protein_file

        output_file_name  = test_file + PROCESSED_TERMINATION
        processed_chains, original_chains_names = self.transform_file(test_file)

        self.generate_table_file(processed_chains, output_file_name)
        header = self.generate_simple_features(processed_chains, output_file_name, original_chains_names, test_file)
